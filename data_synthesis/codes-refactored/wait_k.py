...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--wait-k", type=int, default=3,
                   help="Number of source chunks to read before first WRITE (k=1..9).")
...
def run_one_utterance(row: Dict[str, Any], args: argparse.Namespace,
    ...
    Read the first k non-empty source chunks (READ), then after each
    subsequent chunk, translate and greedily commit the continuation (WRITE).
    Force-complete at the final chunk.
    """
    ...
    for chunk_idx in range(n_chunks):
        ...
        if is_last:
            ...
            # Wait phase: haven't seen k real chunks yet.
