#!/usr/bin/env bash
# ============================================================
# EAST  Stage 2 — Streaming dataset + MetricX input
#
# Input  : ${BASE}/llm_output_merged_fixed
#          ${BASE}/good_train_xl_east_mfa.jsonl
# Output : ${BASE}/streaming_EAST_dataset
#          ${BASE}/metricx_shards/input_00 … input_07  (→ feed to Stage 3)
#
# Submit : sbatch stage2_streaming.sh   (after Stage 1 completes)
# Then   : sbatch --array=0-7 stage3_metricx.sh
# ============================================================
#SBATCH --job-name=east_s2_stream
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH -o /data/user_data/haolingp/data_synthesis/outputs/gigaspeech/train_xl_east/slurm_logs/stage2_stream_%A.out
#SBATCH -e /data/user_data/haolingp/data_synthesis/outputs/gigaspeech/train_xl_east/slurm_logs/stage2_stream_%A.err
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=haolingp@andrew.cmu.edu

set -e
echo "===== EAST Stage 2 — START $(date) ====="
...
# ── Step 3: Build streaming trajectory (chunk = 960 ms) ──────
conda activate SMT
...
# ── Step 3-check: Quality check on streaming dataset ─────────
...
# ── Split into 8 shards (one per GPU) ────────────────────────
