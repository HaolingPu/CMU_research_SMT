...
def batch_get_next_token_distributions(
    ...
    if not prompts:
        return []
    logprobs_n = max(top_k, 100) if min_p > 0 else top_k
    payload = {
        "model": api_model,
        "prompt": prompts,
        "max_tokens": 1,
    ...
    for i in range(len(prompts)):
        ...
        dist, dist_debug = _parse_completion_top_logprobs(logprobs.get("top_logprobs"), tokenizer=tokenizer)
...
def minp_token_ids(dist: Dict[int, float], min_p: float) -> List[int]:
    ...
    return [tok_id for tok_id, prob in sorted(dist.items(), key=lambda kv: kv[1], reverse=True) if prob >= threshold]
...
def choose_consensus_token(
    ...
    best_token = max(intersection, key=lambda tok: sum(d.get(tok, 0.0) for d in distributions) / len(distributions))
    return best_token, {
        "reason": "ok",
...
def extend_pending_tokens(
    ...
    for step_idx in range(max_consensus_steps):
        # Keep the exact token-id continuation state for the LLM probe.
        # This avoids decoding incomplete byte tokens to U+FFFD and feeding
        # garbled text back to the model.
        ...
        for i, (dist, dist_debug) in enumerate(batch_results):
            ...
            candidate_ids = minp_token_ids(dist, min_p) if min_p > 0 else topk_token_ids(dist, candidate_top_k)
            per_future.append({
                "future": futures[i],
        ...
        consensus_token_id, meta = choose_consensus_token(distributions, min_p=min_p, candidate_top_k=candidate_top_k)
        ...
        view = inspect_token_ids(instruct_tokenizer, pending_token_ids)
        log_entry = {
            "step": step_idx,
...
def run_one_utterance(
    ...
    candidate_policy = f"min_p({args.min_p})" if args.min_p > 0 else f"top_k({args.candidate_top_k})"
    ...
    for t in range(len(chunks)):
        ...
        new_committed, delta, committed_delta_token_ids, finalize_meta = finalize_external_commit(
            tokenizer=instruct_tokenizer,
            committed_text=committed_text,
...
def main() -> None:
    ...
    def _process_one_row(row_idx: int, series_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if output_dir is not None:
            write_pretty_json(os.path.join(output_dir, f"{sanitize_filename(result['utt_id'])}.json"), result)
        else:
