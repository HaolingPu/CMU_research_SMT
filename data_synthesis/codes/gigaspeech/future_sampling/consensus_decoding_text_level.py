...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--min-p", type=float, default=0.0,
                   help="Relative min-p threshold (prob >= min_p * max_prob). 0=use top-k.")
...
def build_translation_probe_messages(full_source: str, target_prefix: str, target_lang: str = "Chinese") -> List[Dict[str, str]]:
    user_content = (
        "[TASK]\n"
        f"Translate the [INPUT] text into {target_lang}.\n\n"
        f"[INPUT]\n{full_source}\n\n"
    ...
    user_content += (
        f"A partial {target_lang} translation is already committed. "
    ...
    if not str(committed_text or "").strip():
        messages = [{
            ...
            "content": (
                ...
                f"[INPUT]\n{full_source}\n\n"
                "[IMPORTANT]\n"
                f"Output the complete {target_lang} translation only."
            ),
        }]
        prompt = tokenizer.apply_chat_template(
            messages,
...
def minp_token_ids(dist: Dict[int, float], min_p: float) -> List[int]:
    """Return all token ids whose probability >= min_p * max_prob (relative min-p)."""
    if not dist:
        return []
...
def batch_get_next_token_distributions(
    ...
    prompts = [
        build_translation_probe_prompt(tokenizer, full_source, target_prefix, target_lang=target_lang)
...
def _extract_reference_text_from_row(row: Dict[str, Any], target_lang: str = "Chinese") -> Optional[str]:
    ...
    candidate_keys = []
    if lang_suffix:
        candidate_keys.extend([f"target_full_{lang_suffix}", f"tgt_text_full_{lang_suffix}", f"llm_reference_text_{lang_suffix}"])
