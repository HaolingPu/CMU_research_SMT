...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Future source sampling (baseline1 semantic-LCP) -- dual model."
    )
    ...
    p.add_argument("--score-threshold", type=int, default=80,
                   help="Deprecated for current pipeline (lcp_llm only).")
    ...
    p.add_argument("--judge-prompt-version", choices=["full", "short"], default="short",
                   help="Deprecated for current pipeline (lcp_llm only).")
    p.add_argument("--selection-mode", choices=["lcp_llm"], default="lcp_llm",
                   help="Only mode: lcp_llm (no score-consensus judge).")
...
# ===================================================================
# Semantic LCP: word-boundary trimming of programmatic LCP delta
# ===================================================================

def build_semantic_lcp_prompt(lcp_delta: str) -> str:
    """Ask LLM to trim lcp_delta to the last complete Chinese word/morpheme boundary.

    The programmatic LCP is already computed (character-level upper bound).
    The LLM's only job: find where to safely truncate from the end.
    Output must be a prefix of lcp_delta — cannot add or change characters.
    """
    return (
        "You are a Chinese word-boundary trimmer.\n"
        "Given a Chinese text fragment, output the longest prefix that ends at a "
        "complete word or morpheme boundary.\n\n"
        f'Input: "{lcp_delta}"\n\n'
        "Rules:\n"
        "- Output ONLY the trimmed text. No quotes, no labels, no explanation.\n"
        "- Do NOT add, paraphrase, or translate. You may only remove characters from the END.\n"
        "- If the fragment already ends at a word boundary, output it in full.\n"
        "- A single character is always a valid boundary.\n"
        "- Empty output is valid only if the input itself is empty.\n\n"
        "Examples:\n"
        "  他就是一位著  →  他就是一位       (著 starts 著名, unsafe to stop after 著)\n"
        "  他就是一位著名  →  他就是一位著名  (complete word, output in full)\n"
        "  不可避  →  不可               (避 starts 避免)\n"
        "  不可避免  →  不可避免           (complete word)\n"
        "  这是一个非  →  这是一个          (非 starts 非常 / 非常)\n"
        "  显得单调乏  →  显得单调           (乏 starts 乏味)\n"
    )
...
def get_common_prefix_delta_via_llm(
    ...
    """Compute semantic LCP delta. Returns (delta, used_llm).

    Two-stage approach:
      1. Programmatic LCP → fallback_delta  (character-level upper bound, always valid).
      2. LLM trims fallback_delta to the last complete word boundary → semantic_delta.

    Validation: semantic_delta must be a non-empty prefix of fallback_delta.
    This automatically guarantees committed+semantic_delta is a prefix of every candidate.
    Falls back to fallback_delta (programmatic LCP) when LLM fails.
    No prefill needed: the LLM only needs the delta string, not committed context.
    """
    if not candidate_safe_prefixes:
        return "", False
    committed_norm = normalize_zh(committed) if committed else ""

    # Step 1: Programmatic LCP — character-level upper bound.
    lcp = longest_common_prefix(candidate_safe_prefixes)
    fallback_delta = ""
    if committed_norm:
        if lcp.startswith(committed_norm) and len(lcp) > len(committed_norm):
            fallback_delta = lcp[len(committed_norm):]
    else:
        fallback_delta = lcp

    if not fallback_delta:
        return "", False  # No common extension; skip LLM call.

    # Step 2: Ask LLM to find the semantic word boundary within fallback_delta.
    user_prompt = build_semantic_lcp_prompt(fallback_delta)
    client = _make_sync_client(api_base)
    prompt_text = _build_instruct_generate_prompt(instruct_tokenizer, user_prompt)
    try:
        resp = client.completions.create(
            ...
            prompt=prompt_text,
            temperature=0.0,
            max_tokens=max(32, len(fallback_delta) + 8),
            stop=["\n"],
        )
        raw = (resp.choices[0].text or "").strip()
        semantic_delta = normalize_zh(clean_llm_output(raw))
        # Hard validation: semantic_delta must be a non-empty prefix of fallback_delta.
        # Since fallback_delta is already a prefix of all candidates (minus committed),
        # any prefix of it is automatically valid — no need to re-check all candidates.
        if semantic_delta and fallback_delta.startswith(semantic_delta):
            return semantic_delta, True
    except Exception:
        pass
    return fallback_delta, False
...
def process_one_utterance(
    ...
    for chunk_pos, chunk in enumerate(trajectory):
        ...
        new_chars, lcp_used_llm = get_common_prefix_delta_via_llm(
            ...
            committed_norm, candidate_safe_prefixes,
        )
    ...
    result: Dict[str, Any] = {
        ...
        "config": {
            "version": "baseline1_semantic_lcp_trim",
            "selection_mode": getattr(args, "selection_mode", "lcp_llm"),
            "patches": [
                "word_head_guard",
                "semantic_lcp_trim_over_programmatic_lcp",
            ],
...
# # Baseline1 (semantic LCP trim) - run 10 cases
# INPUT_TSV="$(find /data/user_data/haolingp/data_synthesis -type f -name '*.tsv' | head -n 1)" && \
# echo "Using INPUT_TSV=$INPUT_TSV" && \
# CUDA_VISIBLE_DEVICES=0 python /home/haolingp/CMU_research_SMT/data_synthesis/codes/gigaspeech/future_sampling/llm_future_sampling_b1.py \
#   --input-tsv "$INPUT_TSV" \
#   --output-root /data/user_data/haolingp/data_synthesis/outputs/gigaspeech/train_xl_future_sampling_final/baseline1_semantic_lcp_10 \
#   --task-id 0 \
#   --num-tasks 1 \
#   --max-rows 10 \
#   --selection-mode lcp_llm \
