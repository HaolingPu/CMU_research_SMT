...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--selection-mode", choices=["score_consensus", "lcp_llm"], default="lcp_llm",
                   help="score_consensus: score each candidate + consensus + direction; "
                        "lcp_llm: LLM outputs common-prefix delta only (committed never changed).")
...
# ===================================================================
# LCP-style LLM baseline: common prefix -> delta only (committed never changed)
# ===================================================================

def build_lcp_delta_prompt(
    observed_source: str,
    committed: str,
    candidate_safe_prefixes: List[str],
) -> str:
    """Build prompt for LLM to output the safe common-prefix delta. Committed is fixed; output MUST be only the new delta."""
    candidates_str = "\n".join(
        f"  {i+1}. \"{p}\"" for i, p in enumerate(candidate_safe_prefixes)
    )
    return (
        "You are given M alignment-truncated translation candidates for simultaneous translation.\n\n"
        f'Observed English so far: "{observed_source}"\n'
        f'Chinese already committed (DO NOT change or repeat this): "{committed}"\n\n'
        "Candidate truncated prefixes (each = committed + safe continuation):\n"
        f"{candidates_str}\n\n"
        "Task: Find the longest common prefix (LCP) of these candidates that extends the committed part. "
        "Output ONLY the new delta (the Chinese characters to append after committed). "
        "Do NOT output the committed part. Do NOT modify the committed part.\n\n"
        "Output format (strict): output a single line in one of these forms:\n"
        '  DELTA: <the new Chinese characters only>\n'
        '  or JSON: {"delta": "<the new Chinese characters only>"}\n'
        "If no safe common continuation exists, output: DELTA:\n"
    )


def _parse_lcp_delta_from_response(raw: str, committed_norm: str) -> Optional[str]:
    """Extract delta from LLM response. Ensures we never use model output as committed; only delta is used."""
    text = clean_llm_output(raw).strip()
    if not text:
        return None
    # 1) JSON {"delta": "..."}
    ...
    if json_block:
        try:
            parsed = json.loads(json_block)
            d = parsed.get("delta") if isinstance(parsed, dict) else None
            if d is not None and isinstance(d, str):
                return normalize_zh(d.strip())
        except Exception:
            pass
    # 2) DELTA: ...
    m = re.search(r"DELTA\s*:\s*(.*)", text, re.IGNORECASE | re.DOTALL)
    if m:
        delta = normalize_zh(m.group(1).strip().strip('"').strip("'"))
        return delta if delta else None
    # 3) Model output might be committed+delta; take only the part after committed (committed must not change).
    if committed_norm and text.startswith(committed_norm) and len(text) > len(committed_norm):
        return normalize_zh(text[len(committed_norm):].strip())
    # 4) If no committed, whole output could be delta (first chunk)
    if not committed_norm and text:
        return normalize_zh(text)
    return None


def get_common_prefix_delta_via_llm(
    api_base: str,
    model: str,
    instruct_tokenizer,
    observed_source: str,
    ...
    """Ask LLM for LCP-based delta. Returns (delta, used_llm). If LLM parse fails, fallback to programmatic LCP.

    Committed is never taken from model output; we only use our committed + parsed delta.
    """
    if not candidate_safe_prefixes:
        return "", False
    committed_norm = normalize_zh(committed) if committed else ""
    user_prompt = build_lcp_delta_prompt(observed_source, committed_norm, candidate_safe_prefixes)
    client = _make_sync_client(api_base)
    prompt_text = _build_instruct_generate_prompt(instruct_tokenizer, user_prompt)
    try:
        resp = client.completions.create(
        ...
        raw = (resp.choices[0].text or "").strip()
        delta = _parse_lcp_delta_from_response(raw, committed_norm)
        if delta is not None:
            # Sanity: delta must not contain committed (model might have repeated it)
            if committed_norm and delta.startswith(committed_norm):
                delta = delta[len(committed_norm):].strip()
            return delta, True
    except Exception:
        pass
    # Fallback: programmatic LCP
    lcp = longest_common_prefix(candidate_safe_prefixes)
    if committed_norm and lcp.startswith(committed_norm) and len(lcp) > len(committed_norm):
        return lcp[len(committed_norm):], False
    if not committed_norm and lcp:
        return lcp, False
    return "", False
...
def process_one_utterance(
    ...
    for chunk_pos, chunk in enumerate(trajectory):
        ...
        selection_mode = getattr(args, "selection_mode", "lcp_llm")
        ...
        if selection_mode == "lcp_llm":
            # Step 4 (LCP): LLM outputs common-prefix delta only; committed is never changed.
            candidate_safe_prefixes = [c["safe_prefix"] for c in valid_candidates]
            t4_0 = time.perf_counter()
            new_chars, lcp_used_llm = get_common_prefix_delta_via_llm(
            ...
            timing_totals["step4_lcp_llm_s"] += t4
            _vlog(verbose_log_file,
                  f"  [Step 4 LCP] delta (used_llm={lcp_used_llm}) ({t4:.3f}s): \"{new_chars}\"")
        else:
            # Step 4: LLM scores all valid truncated prefixes
            judge_items = [
                {"candidate_id": j + 1, "safe_prefix": c["safe_prefix"], "delta": c["delta"]}
            ...
            for c, s in zip(valid_candidates, scores):
                ...
                c["judge_tags"] = s["tags"]
            _vlog(verbose_log_file,
                  f"  [Step 4] judge scores ({t4:.3f}s):")
            for c in valid_candidates:
                _vlog(verbose_log_file,
                      f'    idx={c["idx"]} score={c.get("judge_score", 0)} delta="{c["delta"]}"')

            qualified = [
                c for c in valid_candidates
                if int(c.get("judge_score", 0)) >= args.score_threshold
            ]
            qualified_ratio = len(qualified) / max(1, len(valid_candidates))
            _vlog(verbose_log_file,
                  f"  [Step 5] consensus: qualified={len(qualified)}/{len(valid_candidates)} "
                  f"ratio={qualified_ratio:.2f}")

            t5_0 = time.perf_counter()
            if qualified_ratio >= args.consensus_ratio and qualified:
                direction_ok, direction_info = check_direction(
                    [c["delta"] for c in qualified], n=3, min_ratio=0.5,
                )
                _vlog(verbose_log_file,
                      f'  [Step 5] direction_ok={direction_ok} top_key="{direction_info.get("top_key", "")}"')
                if direction_ok:
                    best = max(qualified, key=lambda c: int(c.get("judge_score", 0)))
                    selected_best_idx = int(best["idx"])
                    new_chars = best["delta"]
                else:
                    _vlog(verbose_log_file, "  [Step 5] direction inconsistent; READ")
            t5 = time.perf_counter() - t5_0
            timing_totals["step5_consensus_decision_s"] += t5

        # Guards: min_commit_chars; word_head guard (for lcp_llm always apply when ending on word head)
        risky = _ends_on_word_head(new_chars) and (
            qualified_ratio < 0.85 if selection_mode == "score_consensus" else True
        )
        ...
        if args.save_details:
            ...
            if selection_mode == "lcp_llm":
                detail["selection_mode"] = "lcp_llm"
                detail["lcp_used_llm"] = lcp_used_llm  # False = fallback to programmatic LCP
            else:
                detail["qualified_candidate_indices"] = [c["idx"] for c in qualified]
                detail["qualified_ratio"] = qualified_ratio
                detail["score_threshold"] = args.score_threshold
                detail["consensus_ratio_threshold"] = args.consensus_ratio
                detail["direction_ok"] = direction_ok
                detail["direction_info"] = direction_info
                detail["selected_best_idx"] = selected_best_idx
