...
Experimental JSON-structured-output variant for the thinking-model stage.
...
THINKING_JSON_OUTPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "delta": {
            "type": "string",
            "description": "Chinese text to append directly after committed_translation. Use empty string when action=READ.",
        },
    },
...
FINAL_COMPLETION_JSON_SCHEMA = {
    "type": "object",
    "properties": {
        "continuation": {
            "type": "string",
            "description": "Chinese continuation to append directly after the already committed translation.",
        },
    },
...
def _extract_json_object_from_text(text: str) -> str:
    ...
    if re.search(r"</think>", raw, flags=re.IGNORECASE):
        raw = re.split(r"</think>", raw, flags=re.IGNORECASE)[-1].strip()
...
def _forced_trailing_punct_from_observed(observed_source: str) -> str:
    text = (observed_source or "").rstrip()
    if not text:
        return ""
    for ch in reversed(text):
        if ch in _ASCII_TO_ZH_PUNCT:
            return _ASCII_TO_ZH_PUNCT[ch]
        if ch in _ASCII_TO_ZH_PUNCT.values():
            return ch
        if ch in "\"'”’)]}":
            continue
        break
...
def _extend_safe_prefix_with_forced_punct(
    accepted_full: str,
    candidate_full: str,
    observed_source: str,
) -> str:
    """Restore a trailing punctuation mark when the observed source already forces it.

    Simalign often drops punctuation because punctuation tokens are unaligned.
    If the accepted safe prefix is immediately followed by punctuation in the
    original candidate, and the current observed English boundary already ends
    with a forced punctuation mark, keep that mark.
    """
    expected = _forced_trailing_punct_from_observed(observed_source)
    if not expected:
        return accepted_full
    if len(candidate_full) <= len(accepted_full):
        return accepted_full

    suffix = candidate_full[len(accepted_full):]
    punct_run = ""
    for ch in suffix:
        if ch in "，。！？；：":
            punct_run += ch
    ...
    if not punct_run:
        return accepted_full
    if expected in punct_run:
        keep_len = punct_run.index(expected) + 1
        return accepted_full + punct_run[:keep_len]
    if punct_run[0] == expected:
        return accepted_full + punct_run[0]
    return accepted_full
...
def apply_simalign_delta_check(
    ...
    accepted_full = _extend_safe_prefix_with_forced_punct(
        accepted_full=accepted_full,
        candidate_full=candidate_full,
        observed_source=accumulated_source,
    )
...
def build_thinking_json_request(
    observed_source: str,
    futures: List[str],
    committed_chinese: str,
) -> Dict[str, Any]:
    """Build a structured JSON request for a future structured-output experiment."""
    future_items = [f for f in futures if (f or "").strip()]
    return {
        ...
        "output_contract": {
            "format": "json_object",
            "schema": THINKING_JSON_OUTPUT_SCHEMA,
            "action_meaning": {
                "WRITE": "A non-empty safe Chinese delta can be appended now.",
                "READ": "No safe new Chinese characters can be appended now.",
            },
        },
        "rules": [
            ...
            "Do not assume committed_translation already covers all of observed_source.",
            "Do not revise, replace, or paraphrase already committed_translation. Only append new Chinese characters.",
            "Treat future_continuations as hypothetical noisy samples, not facts. Never commit content that depends on only one sampled future.",
            "The entire delta must remain valid under all future_continuations.",
            "Prefer the longest delta that is strictly supported by observed_source plus all future_continuations.",
            "If futures disagree or look noisy, fall back to the shorter common Chinese prefix or READ.",
            "If a Chinese punctuation mark is already forced by observed_source and remains valid under all future_continuations, include it now.",
        ...
        "examples": [
            ...
            },
            {
                ...
                "output": {"action": "READ", "delta": ""},
            },
            {
                "observed_source": "it was over.",
                "future_continuations": [
                    "Then we left.",
                    "After that, we slept.",
                ],
                "committed_translation": "这件事结束了",
                "output": {"action": "WRITE", "delta": "。"},
            },
            {
                ...
                "output": {"action": "WRITE", "delta": "是编辑，而不是作者"},
            },
        ],
        "state": {
            "observed_source": observed_source,
            "committed_translation": committed_chinese or "",
            "future_continuations": future_items,
        },
    }


def build_thinking_json_prompt(
    observed_source: str,
    futures: List[str],
    committed_chinese: str,
) -> str:
    """Serialize the structured JSON request as the user prompt body."""
    request_obj = build_thinking_json_request(observed_source, futures, committed_chinese)
    return json.dumps(request_obj, ensure_ascii=False, indent=2)


def parse_thinking_json_response(text: str) -> Tuple[str, Dict[str, Any]]:
    """Parse a JSON response like {'action': 'WRITE'|'READ', 'delta': '...'}.

    This helper is intentionally not wired into the main path yet; it is meant
    for structured-output experiments.
    """
    raw = (text or "").strip()
    if not raw:
        return "", {"action": "READ", "delta": "", "raw_text": raw, "error": "empty_response"}

    parsed = json.loads(raw)
    if not isinstance(parsed, dict):
        raise ValueError("Structured thinking response must be a JSON object.")

    action = str(parsed.get("action", "")).upper().strip()
    delta = normalize_zh(str(parsed.get("delta", "") or ""))

    if action not in {"WRITE", "READ"}:
        raise ValueError(f"Invalid structured thinking action: {action!r}")
    if action == "READ":
        delta = ""
    elif not delta:
        raise ValueError("Structured thinking response used WRITE with empty delta.")
...
def build_final_completion_json_request(full_source: str, committed_chinese: str) -> Dict[str, Any]:
    return {
        "task": "complete_remaining_translation",
        "version": "final_completion_json_v1",
        "source_language": "English",
        "target_language": "Chinese",
        "output_contract": {
            "format": "json_object",
            "schema": FINAL_COMPLETION_JSON_SCHEMA,
        },
        "rules": [
            "Return a JSON object only. No prose, no markdown, no code fences.",
            "Produce only the Chinese continuation that should be appended after committed_translation.",
            "Do not repeat already committed_translation.",
            "The completed full translation must be fluent natural Chinese with proper punctuation.",
            "If a punctuation mark is missing exactly at the current boundary, the continuation may begin with that punctuation mark.",
            "If nothing new should be appended, return an empty continuation string.",
        ],
        "state": {
...
def build_final_completion_json_prompt(full_source: str, committed_chinese: str) -> str:
    request_obj = build_final_completion_json_request(full_source, committed_chinese)
    return json.dumps(request_obj, ensure_ascii=False, indent=2)


def parse_final_completion_json_response(text: str) -> Tuple[str, Dict[str, Any]]:
    raw = (text or "").strip()
    if not raw:
        return "", {"continuation": "", "raw_text": raw, "error": "empty_response"}

    parsed = json.loads(raw)
    if not isinstance(parsed, dict):
        raise ValueError("Structured final completion response must be a JSON object.")

    continuation = normalize_zh(str(parsed.get("continuation", "") or ""))
    return continuation, {
...
def call_thinking_model(
    ...
    """Call thinking model via chat endpoint; default to JSON-structured output."""
    ...
    resp = client.chat.completions.create(
        ...
        extra_body={
            ...
            "guided_json": THINKING_JSON_OUTPUT_SCHEMA,
        },
    )
    choice = resp.choices[0]
    message = choice.message
    raw_message_fields = _raw_message_debug_fields(message)
    raw_reasoning = raw_message_fields["message.reasoning_content"] or raw_message_fields["message.reasoning"]
    raw_content = raw_message_fields["message.content"]
    ...
    json_text = _extract_json_object_from_text(raw_content)
    try:
        delta, structured_response = parse_thinking_json_response(json_text)
        content_text = json_text or content_text
    except Exception as e:
        structured_parse_error = str(e)
        fallback_answer = _extract_answer_candidate(content_text)
        delta = "" if (not fallback_answer or fallback_answer.upper() == "EMPTY") else normalize_zh(fallback_answer)
...
def force_complete_translation(
    ...
    messages = [{"role": "user", "content": build_final_completion_json_prompt(full_source, committed_chinese)}]
    resp = client.chat.completions.create(
        model=model,
        messages=messages,
        temperature=0.0,
        ...
        extra_body={
            "chat_template_kwargs": {"enable_thinking": False},
            "guided_json": FINAL_COMPLETION_JSON_SCHEMA,
        },
    )
    choice = resp.choices[0]
    message = choice.message
    ...
    json_text = _extract_json_object_from_text(raw_content)
    try:
        continuation, structured_response = parse_final_completion_json_response(json_text)
        content_text = json_text or content_text
    except Exception as e:
        structured_parse_error = str(e)
        fallback_answer = _extract_answer_candidate(content_text)
        continuation = "" if (not fallback_answer or fallback_answer.upper() == "EMPTY") else normalize_zh(fallback_answer)
...
def process_one_utterance(
    ...
    for chunk_idx, chunk in enumerate(trajectory):
        ...
        if is_last:
            ...
            if final_debug.get("structured_response") is not None:
                _vlog(verbose_log_file, f"  [last] structured_response: {json.dumps(final_debug['structured_response'], ensure_ascii=False)!r}")
            if final_debug.get("structured_parse_error"):
                _vlog(verbose_log_file, f"  [last] structured_parse_error: {final_debug['structured_parse_error']!r}")
            _vlog(verbose_log_file, f"  [last] finish_reason: {final_debug.get('finish_reason')!r}")
        ...
        # --- Thinking model: safe delta ---
        user_content = build_thinking_json_prompt(accumulated_source, futures, committed)
        ...
        if thinking_debug.get("structured_response") is not None:
            _vlog(verbose_log_file, f"  step2_structured_response: {json.dumps(thinking_debug['structured_response'], ensure_ascii=False)!r}")
        if thinking_debug.get("structured_parse_error"):
            _vlog(verbose_log_file, f"  step2_structured_parse_error: {thinking_debug['structured_parse_error']!r}")
        _vlog(verbose_log_file, f"  step2_finish_reason: {thinking_debug.get('finish_reason')!r}")
