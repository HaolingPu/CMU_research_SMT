...
LABEL="${1:?Usage: bash run_gpt_common.sh <label>}"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-/data/user_data/haolingp/data_synthesis/outputs/gigaspeech/consensus_decoding_debug/gpt/${LABEL}}"
...
GPT_SAMPLER_MODEL="${GPT_SAMPLER_MODEL:-gpt-5.4-mini}"
GPT_REASONING_EFFORT="${GPT_REASONING_EFFORT:-medium}"
# Future-sampler prompt format: json = rich (id/text/sense_or_direction/translation_effect/confidence).
# Keep this identical to the DeepSeek run for a clean model A/B.
...
# Pull ONLY OPENAI_API_KEY / OPENAI_API_BASE from a .env if present.
# We deliberately do NOT source the .env wholesale, so that tuning knobs like
# GPT_REASONING_EFFORT keep coming from script defaults (or sbatch --export),
...
for env_file in "${PWD}/.env" "${CODE_DIR}/.env" "/data/user_data/haolingp/.env"; do
  if [[ -f "${env_file}" ]]; then
    ...
    done < <(grep -E '^(OPENAI_API_KEY|OPENAI_API_BASE)=' "${env_file}")
    break
  fi
done

if [[ -z "${OPENAI_API_KEY:-}" ]]; then
  echo "[ERROR] OPENAI_API_KEY is not set; cannot run GPT sampler."
  exit 1
fi
...
INSTRUCT_PID="/tmp/gpt_${LABEL}_instruct_${JOB_ID}_${TASK_ID}.pid"
...
CMD=(
  ...
  --gpt-sampler-model "${GPT_SAMPLER_MODEL}"
...

{
  ...
  echo "gpt_model=${GPT_SAMPLER_MODEL}"
