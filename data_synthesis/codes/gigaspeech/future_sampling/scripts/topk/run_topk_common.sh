...
TOP_K_VALUE="${1:?Usage: bash run_topk_common.sh <top_k> <label>}"
TOP_K_LABEL="${2:?Usage: bash run_topk_common.sh <top_k> <label>}"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-/data/user_data/haolingp/data_synthesis/outputs/gigaspeech/consensus_decoding_debug/topk/consensus_decoding_en_zh_top_${TOP_K_VALUE}}"
...
MIN_CONSENSUS_HORIZON="${MIN_CONSENSUS_HORIZON:-}"
...
INSTRUCT_PID="/tmp/${TOP_K_LABEL}_instruct_${JOB_ID}_${TASK_ID}.pid"
GEMMA_PID="/tmp/${TOP_K_LABEL}_gemma_${JOB_ID}_${TASK_ID}.pid"
QWEN_PID="/tmp/${TOP_K_LABEL}_qwen_${JOB_ID}_${TASK_ID}.pid"
...
echo "===== top-k=${TOP_K_VALUE} (${TOP_K_LABEL}) consensus task ${TASK_ID} ====="
echo "job=${JOB_ID} node=$(hostname)"
echo "rows: start=${ROW_START} count=${ROW_COUNT} offset=${ROW_OFFSET} total=${TOTAL_ROWS}"
echo "output=${RUN_DIR}"
...
CMD=(
  ...
  --candidate-top-k "${TOP_K_VALUE}"
  --min-p "${MIN_P}"
  --num-concurrent-cases "${NUM_CONCURRENT_CASES}"
  --output-jsonl "${RUN_DIR}/results.jsonl"
)

if [[ -n "${MIN_CONSENSUS_HORIZON}" ]]; then
  CMD+=(--min-consensus-horizon "${MIN_CONSENSUS_HORIZON}")
fi
...

{
  ...
  echo "top_k=${TOP_K_VALUE}"
  echo "min_p=${MIN_P}"
  if [[ -n "${MIN_CONSENSUS_HORIZON}" ]]; then
    echo "min_consensus_horizon=${MIN_CONSENSUS_HORIZON}"
  fi
} > "${DONE_FILE}"
