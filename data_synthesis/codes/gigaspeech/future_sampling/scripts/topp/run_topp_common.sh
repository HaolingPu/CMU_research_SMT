...
TOP_P_VALUE="${1:?Usage: bash run_topp_common.sh <top_p> <label>}"
TOP_P_LABEL="${2:?Usage: bash run_topp_common.sh <top_p> <label>}"
...
INSTRUCT_PID="/tmp/topp_${TOP_P_LABEL}_instruct_${JOB_ID}_${TASK_ID}.pid"
GEMMA_PID="/tmp/topp_${TOP_P_LABEL}_gemma_${JOB_ID}_${TASK_ID}.pid"
QWEN_PID="/tmp/topp_${TOP_P_LABEL}_qwen_${JOB_ID}_${TASK_ID}.pid"
...
echo "===== top-p=${TOP_P_VALUE} consensus task ${TASK_ID} ====="
