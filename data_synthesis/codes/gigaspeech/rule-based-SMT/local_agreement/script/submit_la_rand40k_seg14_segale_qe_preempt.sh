...
# Submit dependent LA random-segment[1..4] postprocessing for la_rand40k_seg14:
#   generation -> SEGALE prepare (24 shards) -> SEGALE align 24x
#   -> merge aligned -> MetricX QE prepare -> MetricX QE 24x
#   -> QE filter + length-ratio filter.
#
# Uses full 24-GPU preempt allocation for align/qe arrays.
#
# Usage:
#   bash submit_la_rand40k_seg14_segale_qe_preempt.sh <GEN_JOB_ID>
...
CONSENSUS_ROOT="/data/user_data/haolingp/data_synthesis/outputs/gigaspeech/rule_based_SMT/local_agreement/la_rand40k_seg14"
OUT_ROOT="/data/user_data/haolingp/data_synthesis/outputs/gigaspeech/rule_based_SMT/local_agreement/la_rand40k_seg14_segale"
NUM_DOCS=40000
NUM_SHARDS=24
SYS_ID="la_rand40k_seg14"
QE_THRESHOLD=3.0
MAX_RATIO_REF=1.5
MIN_RATIO_REF=0.7
...
echo "NUM_DOCS=${NUM_DOCS}  NUM_SHARDS=${NUM_SHARDS}"
...
ALIGN_JOB=$(sbatch --parsable \
  ...
  --array=0-23%24 \
  --export="ALL,SHARDS_ROOT=${SHARDS_ROOT},NUM_SHARDS=${NUM_SHARDS}" \
...
MERGE_JOB=$(sbatch --parsable \
  ...
  --job-name=la_seg14_merge \
  -o /data/user_data/haolingp/data_synthesis/outputs/gigaspeech/slurm_logs/la_seg14_merge_%j.out \
  -e /data/user_data/haolingp/data_synthesis/outputs/gigaspeech/slurm_logs/la_seg14_merge_%j.err \
...
QE_PREDICT_JOB=$(sbatch --parsable \
  ...
  --partition=preempt --qos=preempt_qos --requeue \
  --array=0-23%24 \
  --export="ALL,METRICX_RUN_DIR=${METRICX_RUN_DIR},NUM_SHARDS=${NUM_SHARDS}" \
...
echo "Pipeline submitted (la_rand40k_seg14, 24-shard preempt):"
