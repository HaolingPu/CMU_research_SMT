...
# Submit one LA-N=2 pipeline (gen + QE prepare/predict/finalize) for a given
# segment size. Used standalone or by the watchdog.
#
# Usage: bash submit_one_la_pipeline.sh <seg>   e.g. submit_one_la_pipeline.sh 1

SEG="${1:?Usage: bash submit_one_la_pipeline.sh <seg>}"
...
OUTPUT_BASE="/data/user_data/haolingp/data_synthesis/outputs/gigaspeech/rule_based_SMT/local_agreement"
QE_THRESHOLD=3.0

GEN_SBATCH="${SCRIPT_DIR}/run_la_n2_seg${SEG}_50k_8gpu_general.sbatch"
OUTPUT_ROOT="${OUTPUT_BASE}/la_n2_seg${SEG}_50k_general"

gen_id=$(sbatch --parsable "${GEN_SBATCH}")
experiment_dir="${OUTPUT_ROOT}/job_${gen_id}"
metricx_run_dir="${OUTPUT_ROOT}/all_50k-metricx"
filtered_output_dir="${OUTPUT_ROOT}/la_seg${SEG}_qe3"
...
echo "LA seg=${SEG}: ${gen_id} -> ${prep_id} -> ${predict_id} -> ${final_id}"
