#!/usr/bin/env bash
# End-to-end SEGALE sub-sentence QE pipeline for SALAMI de (post fix_llm_raw).
...
ALIGN_JOB=$(sbatch --parsable \
  --export="ALL,SHARDS_ROOT=${SHARDS_ROOT},TASK_LANG=de" \
  -o ${BASE}/slurm_logs/segale_align_%A_%a.out \
  -e ${BASE}/slurm_logs/segale_align_%A_%a.err \
...
MERGE_JOB=$(sbatch --parsable \
  ...
  --gres=gpu:1 --cpus-per-task=2 --mem=8G \
  --job-name=salami_seg_merge_de \
...
QE_JOB=$(sbatch --parsable \
  ...
  -o ${BASE}/slurm_logs/seg_qe_%A_%a.out \
  -e ${BASE}/slurm_logs/seg_qe_%A_%a.err \
...
FIN_JOB=$(sbatch --parsable \
  ...
  --time=00:30:00 \
  --partition=preempt --qos=preempt_qos --requeue \
  --gres=gpu:1 --cpus-per-task=2 --mem=12G \
  --job-name=salami_seg_fin_de \
...
echo
echo "Pipeline submitted (SALAMI de sub-sentence QE):"
