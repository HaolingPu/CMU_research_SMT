...
CKPTS_FILE="${SCRIPT_DIR}/ckpts_ja.txt"
...
REFERENCE_FILE=/data/group_data/li_lab/siqiouya/datasets/acl_6060/dev/text/txt/ACL.6060.dev.en-xx.ja.txt
...
echo "Loaded ${#CKPTS[@]} checkpoints (ja); will score ${#SEGS[@]} latencies each."

for ckpt in "${CKPTS[@]}"; do
    for seg in "${SEGS[@]}"; do
        out_dir="${CKPT_ROOT}/${ckpt}/evaluation/acl_6060/en-ja/seg${seg}"
...
echo
echo "All scoring done. Per-seg scores at <ckpt>/evaluation/acl_6060/en-ja/seg<N>/segmentation_output/scores.tsv"
