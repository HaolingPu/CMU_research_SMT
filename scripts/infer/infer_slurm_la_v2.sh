#!/usr/bin/env bash
# en->zh LA inference v2: same as infer_slurm.sh but with anti-loop sampling.
# Writes to ${MODEL}/evaluation/acl_6060/en-zh-v2/seg<N>/  (separate dir so
# the original v1 outputs at .../en-zh/ are preserved for side-by-side compare).
...
##SBATCH --requeue
#SBATCH --exclude=babel-p9-32,babel-p9-28,babel-m5-32,babel-o5-24,babel-q5-16,babel-n5-32,babel-o5-16,babel-n5-28,babel-s5-24,babel-o5-32
...
# Usage: sbatch infer_slurm_la_v2.sh <MODEL_PATH> <PROMPT_TYPE>
...
OUTPUT_PATH=${MODEL_PATH}/evaluation/acl_6060/en-zh-v2/seg${SOURCE_SEGMENT_SIZE}
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    ...
    if { [ $EXIT_CODE -ne 0 ] && [ ! -s "${OUTPUT_PATH}/instances.log" ]; } \
        ...
        if [ $EXIT_CODE -eq 124 ] || [ $EXIT_CODE -eq 137 ]; then
            echo "simuleval hung past ${ATTEMPT_TIMEOUT}s. Retry $RETRY_COUNT/$MAX_RETRIES..."
        else
