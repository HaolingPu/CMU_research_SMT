...
DEFAULT_AUDIO_CLIPS_ROOT_TEMPLATE = "/data/group_data/li_lab/siqiouya/datasets/gigaspeech/audio_clips_zh_LA-{tag}/"
DEFAULT_OUTPUT_FILENAME_TEMPLATE = "train_s_zh-LA-{tag}.jsonl"
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Convert local-agreement JSON outputs into Swift/Megatron training JSONL."
    )
    p.add_argument("--manifest-root", required=True,
                   help="Directory of per-utterance {utt_id}.json files (post QE+LR filter).")
    p.add_argument("--variant-tag", required=True,
                   help="Short tag used in output filenames and audio clip paths (e.g. 40k).")
...
"""
sbatch \
  --export=ALL,MANIFEST_ROOT=/data/user_data/haolingp/data_synthesis/outputs/gigaspeech/rule_based_SMT/local_agreement/la_rand40k_segale/qe3_lr,VARIANT_TAG=40k \
  /home/haolingp/CMU_research_SMT/scripts/train/run_convert2swift_LA.sbatch
"""
