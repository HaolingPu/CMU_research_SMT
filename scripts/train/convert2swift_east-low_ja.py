"""
en->ja EAST-low recipe: read ONLY low_latency.jsonl (~12.5k post-QE rows),
multiplier uniformly random in [1, 12] per instance.

Outputs:
  audio: /data/group_data/li_lab/siqiouya/datasets/gigaspeech/audio_clips_ja_EAST-low/
  manifest: /data/group_data/li_lab/siqiouya/datasets/gigaspeech/manifests/train_s_ja-EAST-low_origin.jsonl

ja: no space when joining target chunks (same convention as zh).
"""
import os
import json
...
audio_clips_root = "/data/group_data/li_lab/siqiouya/datasets/gigaspeech/audio_clips_ja_EAST-low/"
output_filename  = "train_s_ja-EAST-low_origin.jsonl"
...
instances = []
pbar = tqdm(latency_traj, desc=f"Processing {latency} ja-low, skipped 0")
n_skip = 0
for traj in pbar:
    rows = orig_manifest[orig_manifest['id'] == traj['utt_id']]
    if len(rows) == 0:
        n_skip += 1
        pbar.set_description(f"Processing {latency} ja-low, skipped {n_skip}")
        continue
    ...
    if len(audio_clip_paths) != len(targets):
        n_skip += 1
        pbar.set_description(f"Processing {latency} ja-low, mult={multiplier}, skipped {n_skip}")
        continue
