"""
en->de version of convert2swift_east-mult.py

Reads the de sub-sentence-QE-filtered EAST trajectory tree and emits a swift
SFT manifest in the latency2mult format used by train_EAST-latency2mult_s_de.sh.

Audio source: same English audio TSV as zh/ja (en->* shares the same source audio).
Per-latency multiplier ranges follow the original (low=1, med=2, high=3-12).

Difference vs ja: German targets are joined with spaces (word-level), not
concatenated like ja/zh.
"""
import os
import json
import io
...
# de sub-sentence QE-filtered EAST trajectories.
manifest_root = "/data/user_data/haolingp/data_synthesis/outputs/EAST/gigaspeech_de/segale_qe/final_jsonl_east"
audio_clips_root = "/data/group_data/li_lab/siqiouya/datasets/gigaspeech/audio_clips_de_EAST-latency2mult/"
output_filename = "train_s_de-EAST-latency2mult_origin.jsonl"
...
for latency, (mult_low, mult_high) in latency2multiplier.items():
    ...
    for traj in pbar:
        ...
        for i in range(0, len(traj['target']), multiplier):
            targets.append(" ".join(traj['target'][i:i+multiplier]))  # German: word-level, space-joined
        ...
        messages = [
            {"role": "system", "content": "You are a professional simultaneous interpreter. You will be given chunks of English audio and you need to translate the audio into German text."},
        ]
