"""
en->ja SALAMI (Simul-MuST-C style) — swift manifest builder.

Reads SALAMI offline trajectories from:
  /data/user_data/haolingp/data_synthesis/outputs/SALAMI/gigaspeech_ja/final_jsonl_salami/
    └── {audio_id}/offline_latency.jsonl

Writes audio chunks under:
  /data/group_data/li_lab/siqiouya/datasets/gigaspeech/audio_clips_ja_Simul-MuST-C/
Writes swift manifest:
  /data/group_data/li_lab/siqiouya/datasets/gigaspeech/manifests/train_s_ja-Simul-MuST-C_origin.jsonl

ja: no inter-word space when joining target chunks (same as zh).
"""
import os
import json
...
manifest_root    = "/data/user_data/haolingp/data_synthesis/outputs/SALAMI/gigaspeech_ja/final_jsonl_salami/"
audio_clips_root = "/data/group_data/li_lab/siqiouya/datasets/gigaspeech/audio_clips_ja_Simul-MuST-C/"
output_filename  = "train_s_ja-Simul-MuST-C_origin.jsonl"
...
instances = []
pbar = tqdm(latency_traj, desc=f"Processing {latency} ja, skipped 0")
n_skip = 0
for traj in pbar:
    rows = orig_manifest[orig_manifest['id'] == traj['utt_id']]
    if len(rows) == 0:
        n_skip += 1
        pbar.set_description(f"Processing {latency} ja, skipped {n_skip}")
        continue
    ...
    for i in range(0, len(traj['target']), multiplier):
        targets.append("".join(traj['target'][i:i+multiplier]))  # ja: no space
    ...
    messages = [{"role": "system",
                 "content": "You are a professional simultaneous interpreter. You will be given chunks of English audio and you need to translate the audio into Japanese text."}]
