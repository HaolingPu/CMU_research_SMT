...
# Required env vars:
#   VARIANT_TAG    -- e.g. 40k  (matches convert2swift_PA output naming)
...
VARIANT_TAG="${VARIANT_TAG:?VARIANT_TAG required (e.g. 40k)}"
EXP_NAME=gigaspeech-zh-PA-${VARIANT_TAG}-s-bsz4
...
apptainer exec \
  ...
  bash -c '
export train_dataset=/data/group_data/li_lab/siqiouya/datasets/gigaspeech/manifests/train_s_zh-PA-${VARIANT_TAG}.jsonl
