#!/usr/bin/env bash
# en->de SALAMI (Simul-MuST-C style) training.
# Reads train_s_de-Simul-MuST-C_origin.jsonl (produced by convert2swift_salami_ja.py).
...
apptainer exec \
  ...
  bash -c '
export train_dataset=/data/group_data/li_lab/siqiouya/datasets/gigaspeech/manifests/train_s_de-Simul-MuST-C_origin.jsonl
...
megatron sft \
    ...
    --clip_grad 1.0 \
    --max_epochs 1 \
    --save /data/user_data/haolingp/ckpts/infinisst-omni/gigaspeech-de-Simul-MuST-C-s_origin-bsz4_de \
    --log_interval 10 \
    --eval_interval 200 \
    --save_interval 200 \
    --max_length 2048 \
    ...
    --wandb_exp_name gigaspeech-de-Simul-MuST-C-s_origin-bsz4_de

BASE_DIR=/data/user_data/haolingp/ckpts/infinisst-omni/gigaspeech-de-Simul-MuST-C-s_origin-bsz4_de
LATEST_CKPT=$(ls -td "$BASE_DIR"/v*-* 2>/dev/null | head -n 1)

if [ -z "$LATEST_CKPT" ]; then
    echo "Warning: No checkpoint found for SALAMI de"
    exit 1
fi
