...
# Hibiki zh reproduction -- independent retrain in haolingp env to verify the
# mentor's strong hibiki result (figure: BLEU 42->47) is real, not a bad run.
# Same data as the mentor's ckpt (train_s_zh-hibiki.jsonl), but audio paths
# remapped to audio_clips_zh_hibiki_backup (the original clips were 66% deleted;
# backup is intact). Megatron hyperparams mirror train_LA_s.sh (proven recipe);
# no MEGATRON_LM_PATH (container bundles Megatron). EXP_NAME matches the figure
# label so it slots back onto the LongYAAL plot.
...
EXP_NAME=gigaspeech-zh-hibiki-s-bsz4
TRAIN_DATASET=/data/group_data/li_lab/haolingp/data_synthesis/gigaspeech/hibiki/train_s_zh-hibiki_backup-audio.jsonl
